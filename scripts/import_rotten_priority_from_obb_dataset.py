...
def _find_label_dir(split_dir: Path) -> Optional[Path]:
    # Standard roboflow layout: split/labels/
    d = split_dir / "labels"
...
def _obb_to_xyxy_pixels(
    ...
    *,
    ...
    # "+5%" requested: interpret as +5% total size → pad 2.5% each side.
    ...
    for split in splits:
        ...
        for img_path in _iter_images(images_dir):
            ...
            for obj_i, raw in enumerate(txt.splitlines()):
                ...
                base_name = f"rottenobb__{split}__{img_path.stem}__obj{obj_i:03d}__cls{cid}.jpg"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Import ROT/DISEASE crops from YOLOv8-OBB dataset into classifier priority queue.")
    ap.add_argument(
        "--dataset-root",
        type=Path,
        default=REPO_ROOT / "data" / "strawberry rotten.v1i.yolov8-obb",
    )
    ap.add_argument("--out-dir", type=Path, default=REPO_ROOT / "data" / "classifier_priority_queue" / "rotten")
    ap.add_argument("--pad-total-frac", type=float, default=0.05, help="Total bbox padding fraction (0.05 = +5% total size).")
    ap.add_argument(
        "--include-classes",
        default="0,1,3",
        help="Comma-separated class ids to include as 'rotten' priority. Default includes diseased classes (excludes healthy=2).",
    )
    ...
    ap.add_argument(
        "--report-path",
        type=Path,
        default=REPO_ROOT / "data" / "classifier_priority_queue" / "reports" / "import_rotten_from_obb_report.txt",
    )
    args = ap.parse_args(list(argv) if argv is not None else None)

    splits = [s.strip() for s in str(args.splits).split(",") if s.strip()]
    include_classes = {int(x.strip()) for x in str(args.include_classes).split(",") if x.strip()}

    stats = import_rotten_crops(
    ...
    lines = [
        "IMPORT ROTTEN PRIORITY CROPS (YOLOv8-OBB -> axis-aligned bbox)",
        time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime()),
        "",
        f"dataset_root: {Path(args.dataset_root).resolve()}",
        f"splits: {', '.join(splits)}",
        f"include_classes: {sorted(include_classes)}",
        ...
        f"included_objects_seen: {stats.included_objects_seen}",
