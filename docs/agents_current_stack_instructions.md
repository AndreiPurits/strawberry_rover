...
- Зона guard — **конус ~40° строго перед ровером** (±20° от оси). Точки **вне конуса полностью игнорируются** для STOP.
- STOP срабатывает только если **внутри конуса** есть точка ближе `0.40 m` (release — `0.48 m`, latch/hysteresis).
- Назад — разрешено. Override (Shift / E / Space / R2) — проезд вперёд при зажатии.
