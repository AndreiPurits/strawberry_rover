...
6. Фиксация выбранной ягоды в кадре камеры на манипуляторе: повторный вывод в ROI, удержание при кратковременной потере детекции, **исключение клешней** из области детекции.
7. Одношаговый подход: выбор ближайшего успешного демонстрационного перемещения, коррекция остатка ошибки **локальным якобианом** `[dpx, dpy, ddepth] ← [dshoulder, delbow, dbase]`, одно движение в пространстве суставов, проверка достижения рабочей дистанции (standoff).
8. Вариант осуществления: декартово позиционное визуальное серво с онлайн-оценкой отображения смещения рабочего органа в смещение цели в камере.
9. Передача точки реза исполнительным устройствам. Навигация шасси и операторский слой в объём изобретения не входят.

Описание способа подхода: `docs/manipulator_approach.md`.
...
| Фиксация цели, одношаговый подход, проверка standoff | `pipelines/arm_approach/` |
| Параметры подхода | `config/arm_approach_v1.yaml` |
| Геометрия подхода (демо-метрика, якобиан, карта tip→cam) | `scripts/arm_approach_geometry.py` |
| Офлайн демо-проход подхода | `scripts/run_arm_approach_field_demo.py` |
| Юнит-тесты подхода | `tests/test_arm_approach_v1.py` |
| Описание способа подхода | `docs/manipulator_approach.md` |
| Контракт подхода | `reports/manipulator_approach/method.md` |
