...
- Выполнен MVP-fix бесконечного Auto traversal (минимальная правка FSM, без Stage 6.4):
  - убрана терминальность `FINISHED` для штатного Auto-прохода;
  - в конце каждого ряда FSM всегда выбирает следующий ряд по детерминированному циклу;
  - на краях поля направление прохода по рядам автоматически реверсируется (snake-петля вверх/вниз по индексам рядов);
  - Auto-движение теперь рассчитано на бесконечный повторяемый loop по известному bed-маршруту.
